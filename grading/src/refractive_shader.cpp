        ...
        const vec3& same_side_normal, int recursion_depth,bool is_exiting) const
{
	...
	double cos_r 		 = sqrt(1 - ni_nr_sqr * (1 - pow(cos_i, 2)));
	...
	if(cos_r_sqr < 0) { //check for total internal reflection
		...
		double r_para_sqr, r_perp_sqr;

		r_parallel = (n_r * cos_i - n_i * cos_r) / (n_r * cos_i + n_i * cos_r);
		r_para_sqr = pow(r_parallel, 2);
		r_perpendicular = (n_i * cos_i - n_r * cos_r) / (n_i * cos_i + n_r * cos_r);
		r_perp_sqr = pow(r_perpendicular, 2);
		reflectance_ratio = (r_para_sqr + r_perp_sqr) * 0.5;
	}
    }
